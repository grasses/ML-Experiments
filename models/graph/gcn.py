...
from .ops import numpy_to_graph
...
class GCN(nn.Module):
    def __init__(self, in_dim, out_dim,
                 hidden_dim=[64, 32, 16],  # GNN layers + 1 layer MLP
                 dropout=0.2,
                 activation=F.relu):
    ...
    def forward(self, data):
        # data：features, adj, mask, num_node, labels
        batch_g = []
        for idx, adj in enumerate(data[1]):
    ...
    def feature_list(self, data):
        ...
        # B=batch_size, N=num_nodes, F=num_features
        B, N, F = data[0].shape[:3]
        x = data[0].reshape(B * N, F)
        mask = mask.reshape(B * N, 1)
        out_list = []
        for layer in self.layers:
            x = layer(batch_g, x)
            x = x * mask
            out_list.append(x.contiguous().view(B, -1).clone().detach().cpu())
